...
namespace SimpleGE::Network
{
  ...
  void Connection::RecvRaw(gsl::span<std::byte> buffer) const
  {
    auto bytesReceived = ::recv(clientSocket, std::bit_cast<char*>(buffer.data()), buffer.size(), 0);
  ...
  void Connection::SendRaw(gsl::span<std::byte> data) const
  {
  ...
  void Server::Service()
  {
    ...
    if (::getnameinfo((::sockaddr*) &client, sizeof(client), host.data(), host.size(), service.data(), service.size(),
                      0) == 0)
    {
    ...
    {
      ::inet_ntop(AF_INET, &client.sin_addr, host.data(), host.size());
